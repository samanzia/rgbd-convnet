...
function Knettest(args=ARGS)
    ...
    @add_arg_table s begin
        ...
        ("--nbatch"; arg_type=Int; default=80)
        ("--lr"; arg_type=Float64; default=0.00001)
        ("--epochs"; arg_type=Int; default=4)
    ...
    for epoch=1:epochs
        fileCount = 1;
        fileName = "TrainShuffleSplit1”;
        variableName = "TrainSplit1Batch”;
        ytrnFile = matopen("TrainShuffleSplit1Labels.mat")
        ytrn = read(ytrnFile,"TrainSplit1LabelsMat")
        ytrn = map(Float32,ytrn)

        for i=1:24
            xtrnFile = matopen(fileName * i * “.mat”)
            xtrn = read(xtrnFile,variableName*i)
            ...
            ytrn = read(ytrnFile,"TrainSplit1LabelsMat")
            ytrn = map(Float32,ytrn)
            ytrn1 = ytrn[:,fileCount:fileCount+39999]
            println(size(ytrn1))

            mtrn = 255;
            xtrn = xtrn/mtrn;
            dtrn = minibatch(xtrn, ytrn1, nbatch)
            train(threeD,dtrn,softloss;losscnt=fill!(l,0), maxnorm=fill!(m,0))
            atrn = 1-test(threeD,dtrn,zeroone)
            println((epoch, atrn))
            xtrn = 0; dtrn = 0; ytrn1 = 0;

            fileCount = fileCount + 40000;

     	end
	
	JLD.save("VGG.jld", "model", clean(vgg));
    return (l[1]/l[2],m...)
end
