...
function Knettest(args=ARGS)
	...
	xtrnFile = matopen("TrainShuffle1Split1.mat")
	xtrn = read(xtrnFile,"TrainSplit1")
    xtrn = map(Float32,xtrn)
	println("Loading xtrn")

	xtstFile = matopen("TestShuffle1Split1.mat")
	...
	ytrnFile = matopen("TrainShuffle1Split1LabelsMat.mat")
	ytrn = read(ytrnFile,"TrainSplit1LabelsMat")
    ytrn = map(Float32,ytrn)
	
	ytstFile = matopen("TestShuffle1Split1LabelsMat.mat")
	...
	#	adev = 1-test(softm,ddev,zeroone)
		...
		if(prevdev < atst)
			...
			prevdev = atst;
			println("Best Performance saved");
		end
